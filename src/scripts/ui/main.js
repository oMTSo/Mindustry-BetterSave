...
const cloudSettingDialog = require("bettersave/ui/cloudSetting");
...
exports.init = () => {
    ...
    let updateButtons = () => {
        ...
        let width = 210;
        // Dynamically calculate button width if on mobile/narrow screen
        // We have 3 buttons max (Back, Upload, Download). 
        // If 3 * 210 > ScreenWidth, we need to shrink.
        if (cloud.isEnable()) {
            let screenW = Core.graphics.getWidth();
            let totalNeeded = 3 * 210 * Packages.arc.scene.ui.layout.Scl.scl(1.0);
            if (screenW < totalNeeded) {
                // Calculate width: (ScreenW / Scale - Padding) / 3
                // Padding: 40px total margins (20px left + 20px right)
                width = (screenW / Packages.arc.scene.ui.layout.Scl.scl(1.0) - 40) / 3;
            }
        }

        // mainDialog.dialog.addCloseButton(); 
        // Manually add close button to control its size
        mainDialog.dialog.buttons.button("@back", Icon.left, () => {
            mainDialog.dialog.hide();
        }).size(width, 64);

        if (cloud.isEnable()) {
            mainDialog.button("@cloudConfig.upload", Icon.upload, () => {
            ...
            mainDialog.button("@cloudConfig.download", Icon.download, () => {
